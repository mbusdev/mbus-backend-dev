...
describe('Reminders', () => {
    it('should not send a threshold immediately and should move to next stage after sending', () => {
        ...
        expect(remindersLater.reminder.get(Array.from(remindersLater.reminder.keys())
            .find((thresholdEvent) => r.sameBaseEvent(r.fromKey(thresholdEvent), testEvent))).has(testToken))
            .toBe(true);
    ...
    it('should get unix timestamp from ride bus api', () => __awaiter(void 0, void 0, void 0, function* () {
        configDotenv();
        ...
        const res = yield client.get('/gettime', { params: { unixTime: true } });
        expect(Math.abs(parseInt(res.data["bustime-response"]["tm"]) - Date.now())).toBeLessThan(60 * 1000);
    }));
    it('should have cached preds in a good state', () => __awaiter(void 0, void 0, void 0, function* () {
        // simulate one cycle of the core jobs
        yield initializeRoutes();
        yield rebuildGraph();
        yield updateBusPositions();
        // are there preds?
        ...
        const sample = { rt: "", stpid: "", vid: "", prdtm: 0, prdctdn: "" };
        const allThere = (x) => {
