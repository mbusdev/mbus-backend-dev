...
function optimizeWalkingFastest(journey: any, cachedGraph: any): any {
  ...
  for (let i = 0; i < journey.legs.length; i++) {
    ...
    if (leg.trip && i > 0 && !journey.legs[i - 1].trip) {
      ...
      const destIdx = trip.stopTimes.findIndex(st => st.stop === tripLeg.destination);

      let bestStop = originalBoardStop;
      let bestIdx = boardIdx;
      let bestDistance = transfer.duration;
      const originTransfers = cachedGraph.transfers[transfer.origin];

      console.log("hi");
      if (!originTransfers || originTransfers.length === 0) {
        optimizedLegs.push(leg);
        continue;
      }
      const walkStartTime = transfer.startTime;
      for (let j = Math.max(0, boardIdx-3); j < Math.min(trip.stopTimes.length, boardIdx+3); j++) {
        const candidate = trip.stopTimes[j];
        const busArrival = candidate.arrivalTime
        // Look for a matching transfer
        let transferDuration: number | null = null;
        for (const t of originTransfers) {
          if (t.destination === candidate.stop) {
            transferDuration = t.duration;
            break;
          }
        }

        if (transferDuration === null) continue; // no transfer available to this stop
        const walkArrival = walkStartTime + transferDuration;

        // console.log(
        //   `Checking stop=${candidate.stop} j=${j}, ` +
        //   `duration=${transferDuration}, walkArrival=${walkArrival}, busArrival=${busArrival}, ` +
        //   `originalBoardStop=${originalBoardStop}`
        // );

        if (walkArrival <= busArrival + 60 && transferDuration < bestDistance) {
          bestStop = candidate.stop;
          bestIdx = j;
          bestDistance = transferDuration;
        }
      }

      if (bestStop !== originalBoardStop) {
        console.log(`Optimized walking to ${bestStop} instead of ${originalBoardStop}, saving ${Math.round(bestDistance)} seconds`);
        // Update transfer
        transfer.destination = bestStop;
        transfer.duration = bestDistance;

        // Trim trip leg to start from bestStop
        tripLeg.stopTimes = trip.stopTimes.slice(bestIdx);
        tripLeg.origin = bestStop;
